...
const ALLOWED_WIDTHS = [72, 80, 96, 144, 160, 288, 320, 400, 640, 800, 960, 1152, 1280] as const;
const ALLOWED_QUALITIES = [60, 70, 75, 80, 85, 90] as const;
...
const PRESET_WIDTHS: Record<Exclude<ImagePreset, "original">, readonly number[]> = {
  thumbnail: [72, 144, 288],
  card: [320, 640, 960],
  detail: [640, 800, 960, 1280],
  avatar: [80, 160, 320],
};
...
export function imageUrl(
  ...
  const w = allowlistedWidth((width ?? config.width) * clampDpr(dpr));
  const h = Math.round(config.height * clampDpr(dpr));
  const q = allowlistedQuality(quality ?? config.quality);

  if (CDN_MODE === "imgproxy") {
    return imgproxyUrl(src, w, h, q);
  }

  return cloudflareUrl(src, w, h, q);
}
...
export function imageSrcSet(
  ...
  const candidates = widths ?? PRESET_WIDTHS[preset];
  return [...new Set(candidates.map(allowlistedWidth))]
    .sort((left, right) => left - right)
    .map((candidate) => `${imageUrl(src, preset, 1, candidate, quality)} ${candidate}w`)
    .join(", ");
}
...
function cloudflareUrl(src: string, w: number, h: number, q: number): string {
  ...
  return `/cdn-cgi/image/${params}/${encodeURIComponent(src)}`;
}
...
function allowlistedWidth(width: number): number {
  const requested = Number.isFinite(width) ? Math.max(0, Math.round(width)) : 0;
  const ceiling = ALLOWED_WIDTHS.find((candidate) => candidate >= requested);
  return ceiling ?? ALLOWED_WIDTHS[ALLOWED_WIDTHS.length - 1];
}

function allowlistedQuality(quality: number): number {
  const requested = Number.isFinite(quality) ? Math.max(0, Math.round(quality)) : 0;
  return ALLOWED_QUALITIES.reduce((closest, candidate) =>
