...
describe("request", () => {
  ...
  it("coalesces concurrent 401s behind one refresh even when the second response arrives late", async () => {
    liveToken = "old-jwt";
    ...
    secondary401.promise = new Promise<Response>((resolve) => {
      secondary401.resolve = resolve;
    });
    ...
    const releaseSecondary401 = secondary401.resolve;
    if (!releaseSecondary401) throw new Error("Expected the secondary 401 resolver to be captured");
    releaseSecondary401(new Response("unauthorized", { status: 401 }));

    await expect(second).resolves.toEqual({ id: "p2", name: "Secondary" });
    expect(refreshTokensMock).toHaveBeenCalledTimes(1);
    expect(liveToken).toBe("new-jwt");
