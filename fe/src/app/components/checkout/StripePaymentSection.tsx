...
export function StripePaymentSection({
 ...
 useEffect(() => {
 if (!stripeEnabled || !stripePromise) return;
 ...
 }, [orderId, idempotencyKey, clientSecret, loading, stripeEnabled, stripePromise]);
 ...
 if (error) {
 return <div className="mt-3 rounded-2xl border-2 border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>;
 }
