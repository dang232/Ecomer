...
import { type ReactNode as RLNode } from "react";
...
vi.mock("../hooks/use-orders", () => ({
  ...
  myOrdersOptions: vi.fn(() => ({ // returns queryOptions-like object
    queryKey: ["orders"],
...
function makePendingOrder(id: string) {
  return {
    id,
    status: "PENDING",
    subOrders: [{ id: "so-" + id, sellerId: "seller-1", sellerName: "Test Seller", items: [{ productId: "p-1", name: "Product 1", quantity: 1, price: 100000 }] }],
    total: 100000,
...
describe("OrdersPage — P0-9 cancel confirm dialog", () => {
  ...
  it("opens the confirm dialog when the cancel button is clicked", async () => {
    ...
    render(<MemoryRouter><OrdersPage /></MemoryRouter>, { wrapper: Wrapper });
    ...
    await waitFor(() => { expect(screen.getByRole("button", { name: /orders.actions.cancel/i })).toBeInTheDocument(); });
  ...
  it("does NOT call cancelOrder until the confirm button in the dialog is clicked", async () => {
    ...
    const confirmBtn = within(dialog).getAllByRole("button").find((b) => b.className.includes("bg-error"));
    expect(confirmBtn).toBeTruthy();
    fireEvent.click(confirmBtn!);

    await waitFor(() => { expect(cancelOrderMock.mock.calls.some((args) => args[0] === "ord-cancel-2")).toBe(true); });
  });
