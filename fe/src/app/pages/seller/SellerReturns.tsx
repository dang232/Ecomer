import { IconAlertCircle, IconCircleCheck, IconPackage, IconRotate } from "@tabler/icons-react";
...
const STATUS_TABS = [
  { id: "pending", match: (s: string) => s.toUpperCase() === "REQUESTED" },
  { id: "approved", match: (s: string) => s.toUpperCase() === "APPROVED" },
  { id: "completed", match: (s: string) => s.toUpperCase() === "COMPLETED" },
  { id: "rejected", match: (s: string) => s.toUpperCase() === "REJECTED" },
] as const;
...
export function SellerReturns() {
  ...
  const returns = useMemo<Return[]>(() => returnsQuery.data ?? [], [returnsQuery.data]);
  const filtered = useMemo(
    () => returns.filter((r) => STATUS_TABS.find((s) => s.id === tab)?.match(r.status) ?? true),
    [returns, tab],
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <FormDialog
        ...
        onSubmit={({ reason }) => {
          if (rejectFor) reject.mutate({ returnId: rejectFor, reason });
        }}
      ...
      {!returnsQuery.isLoading && returns.length === 0 && !returnsQuery.error ? (
      ...
      {!returnsQuery.isLoading && returns.length > 0 && filtered.length === 0 ? (
                ...
                <div key={ret.id} className="px-5 py-4 flex items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <IconRotate size={14} className="text-muted-foreground" aria-hidden="true" />
