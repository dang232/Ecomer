...
it.each([
  ...
  try {
    ...
    expect(() => loadConfig().createKafkaClientConfig("notification-test")).toThrow(name);
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
});
...
it("fails closed when the broker list is empty", () => {
  ...
  try {
    ...
    expect(() => loadConfig().createKafkaClientConfig("notification-test")).toThrow("KAFKA_BOOTSTRAP_SERVERS");
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
});
