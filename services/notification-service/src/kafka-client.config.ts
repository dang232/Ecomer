...
export function createKafkaClientConfig(clientId: string): KafkaConfig {
  const brokers = (process.env.KAFKA_BOOTSTRAP_SERVERS ?? (production ? '' : 'localhost:9092')).split(',').map((value) => value.trim()).filter(Boolean);
  if (brokers.length === 0) throw new Error('KAFKA_BOOTSTRAP_SERVERS must contain a broker');
