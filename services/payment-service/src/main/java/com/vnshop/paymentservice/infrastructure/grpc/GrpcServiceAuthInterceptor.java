...
final class GrpcServiceAuthInterceptor implements ServerInterceptor {
    ...
    private static final Metadata.Key<String> SERVICE_ID = Metadata.Key.of(
            SERVICE_ID_HEADER, Metadata.ASCII_STRING_MARSHALLER);
    private static final Metadata.Key<String> SERVICE_TOKEN = Metadata.Key.of(
            SERVICE_TOKEN_HEADER, Metadata.ASCII_STRING_MARSHALLER);

    private final String expectedServiceId;
    private final byte[] expectedToken;

    GrpcServiceAuthInterceptor(String expectedServiceId, String expectedToken) {
        if (expectedServiceId == null || expectedServiceId.isBlank()
    ...
    public <RequestT, ResponseT> Listener<RequestT> interceptCall(
        ...
        String serviceToken = headers.get(SERVICE_TOKEN);
        if (!expectedServiceId.equals(serviceId)
                || serviceToken == null
                || !MessageDigest.isEqual(
                        expectedToken, serviceToken.getBytes(StandardCharsets.UTF_8))) {
