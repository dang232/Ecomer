  ...
  group('CartBloc', () {
    ...
    group('Add Item to Cart', () {
      blocTest<CartBloc, CartState>(
        ...
        expect: () => [
          ...
          isA<CartState>()
              .having((s) => s.cart?.items.length, 'item count', 3),
    ...
    group('Update Quantity', () {
      blocTest<CartBloc, CartState>(
        ...
        build: () {
          ...
          when(() => mockRepository.updateItemQuantity(any(), any()))
              .thenAnswer((_) async => updatedCart);
          when(() => mockRepository.isOnline).thenReturn(true);
          return CartBloc(repository: mockRepository);
        },
        ...
        verify: (_) {
          verify(() => mockRepository.updateItemQuantity('item_1', 5)).called(1);
        },
      );
      ...
      blocTest<CartBloc, CartState>(
        ...
        expect: () => [
          ...
          isA<CartState>()
              .having((s) => s.cart?.items.length, 'item count', 1),
          isA<CartState>()
              .having((s) => s.cart?.items.length, 'item count', 1),
        ],
      );
    ...
    group('Offline Queue Operations', () {
      blocTest<CartBloc, CartState>(
        ...
        build: () {
          when(() => mockRepository.syncPendingOperations())
              .thenAnswer((_) async {});
          when(() => mockRepository.getCart())
              .thenAnswer((_) async => testCart);
          when(() => mockRepository.isOnline).thenReturn(true);
