...
- Todo 3 follow-up decision (2026-08-21): Java clients retain explicit JKS truststore/keystore contracts; KafkaJS clients retain PEM contracts. Inventory records both formats and every workload must bind the format consumed by its runtime.
- Todo 3 regression decision (2026-08-21): inventory clients are the complete runtime identity authority, including kafka-admin-bootstrap; TLS format is explicit per client and validator checks rendered workload bindings rather than trusting declarations alone.
